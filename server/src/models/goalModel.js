...
class Goal {
    static async create(goalData) {
        const { goalType, startDate, endDate, targetCalories, targetProtein, targetCarbs, targetFat, uid } = goalData;
        
        return new Promise((resolve, reject) => {
            const query = `
                INSERT INTO GOAL (Goal_Type, Start_Date, End_Date, Target_Calories, Target_Protein, 
                                Target_Carbs, Target_Fat, UID)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `;
            
            db.run(query, [goalType, startDate, endDate, targetCalories, targetProtein, 
                          targetCarbs, targetFat, uid], function(err) {
                if (err) reject(err);
                resolve(this.lastID);
            });
        });
    ...
    static async update(gid, goalData) {
        const { goalType, startDate, endDate, targetCalories, targetProtein, targetCarbs, targetFat } = goalData;
        ...
        return new Promise((resolve, reject) => {
            ...
            db.run(query, [goalType, startDate, endDate, targetCalories, 
                          targetProtein, targetCarbs, targetFat, gid], function(err) {
                if (err) reject(err);
